 ...
 module Zip
  class Entry
    ...
    def initialize(
      ...
      @time               = case time
                            when ::BimTools::Zip::DOSTime
                              time
                            when Time
                              ::BimTools::Zip::DOSTime.from_time(time)
                            else
                              ::BimTools::Zip::DOSTime.now
                            end
      @extra              =
    ...
    def check_c_dir_entry_static_header_length(buf)
      return unless buf.nil? || buf.bytesize != ::BimTools::Zip::CDIR_ENTRY_STATIC_HEADER_LENGTH
    ...
    def read_extra_field(buf, local: false)
      if @extra.kind_of?(::BimTools::Zip::ExtraField)
        @extra.merge(buf, local: local) if buf
      else
        @extra = ::BimTools::Zip::ExtraField.new(buf, local: local)
      end
    end
    ...
    def get_extra_attributes_from_path(path) # :nodoc:
      ...
      return if ::BimTools::Zip::RUNNING_ON_WINDOWS
    ...
    def write_to_zip_output_stream(zip_output_stream) #:nodoc:all
      if @ftype == :directory
        ...
        get_input_stream do |is|
          ::BimTools::Zip::IOExtras.copy_stream(zip_output_stream, is)
        end
      else
    ...
    def set_time(binary_dos_date, binary_dos_time)
      ...
      warn 'WARNING: invalid date/time in zip entry.' if ::BimTools::Zip.warn_invalid_date
    end
