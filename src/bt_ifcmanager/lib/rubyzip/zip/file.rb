  ...
  class File
    ...
    def initialize(path_or_io, create: false, buffer: false, **options)
      ...
      options  = DEFAULT_RESTORE_OPTIONS
                 .merge(compression_level: ::BimTools::Zip.default_compression)
                 .merge(options)
    ...
    class << self
      # Similar to ::new. If a block is passed the BimTools::Zip::File object is passed
      # to the block and is automatically closed afterwards, just as with
      # ruby's builtin File::open method.
      def open(file_name, create: false, **options)
        zf = ::BimTools::Zip::File.new(file_name, create: create, **options)
      ...
      def open_buffer(io = ::StringIO.new, create: false, **options)
        unless IO_METHODS.map { |method| io.respond_to?(method) }.all? || io.kind_of?(String)
          raise 'BimTools::Zip::File.open_buffer expects a String or IO-like argument' \
    ...
    # of BimTools::Zip::Entry, a new BimTools::Zip::Entry will be initialized using the arguments
    ...
    def add(entry, src_path, &continue_on_exists_proc)
      ...
      new_entry = if entry.kind_of?(::BimTools::Zip::Entry)
                    entry
                  else
                    ::BimTools::Zip::Entry.new(
                      @name, entry.to_s,
    ...
    def add_stored(entry, src_path, &continue_on_exists_proc)
      entry = ::BimTools::Zip::Entry.new(
        @name, entry.to_s, compression_method: ::BimTools::Zip::Entry::STORED
      )
    ...
    def mkdir(entry_name, permission = 0o755)
      ...
      @cdir << ::BimTools::Zip::StreamableDirectory.new(@name, entry_name, nil, permission)
    end
