...
Textbox.add = function( properties ) {
  ...
  UI.add_event( 'click', $control, $textbox );
  // Attach to document.
  control.attach();
  return control;
}
...
Textbox.set_options = function( ui_id, value, min_length ) {
  $control = $('#' + ui_id);
  $textbox = $control.children('input');
  $( '#' + ui_id + '_ui' ).autocomplete( {minLength: min_length, autoFocus: true, source: value});
  // Alleen waarden toestaan die in de lijst staan: "change: function (event, ui) { if(!ui.item){ $('#" + @id + "').val(''); } }"
  // Lijst uitklappen zonder dat er iets is ingevoerd: "$( '#" + @id + "' ).on('click focus',function() { $(this).autocomplete('search', ''); });"
  return value;
};
